...
#   3. A deploy drops a world-writable file under /srv/webapp                     [CIS-6.1.10]
...
inject() {
  ...
  if grep -q 'net.ipv4.conf.all.rp_filter' /etc/sysctl.d/99-cis-hardening.conf 2>/dev/null; then
  ...
  echo "  [3/3] deploy drop: world-writable /srv/webapp/upload.tmp (CIS-6.1.10)"
  mkdir -p /srv/webapp
  echo "transient deploy artifact" > /srv/webapp/upload.tmp
  chmod 0666 /srv/webapp/upload.tmp
