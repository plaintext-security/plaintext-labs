...
The corpus and IOCs are anchored to the Log4Shell (CVE-2021-44228) exploitation wave.
...
def decide_tools(question: str) -> list[tuple[str, dict]]:
    ...
    for domain in DOMAIN_RE.findall(question):
        if "." in domain and not domain.endswith(".internal"):
    ...
    alert_keywords = ["alert", "host", "WIN-", "SRV-", "incident", "open"]
    if any(kw.lower() in question.lower() for kw in alert_keywords):
        # Extract a search term (host names like WIN-WKS-023 or SRV-DB01)
        match = re.search(r'\b(?:WIN|SRV)-[A-Z0-9]+(?:-\d+)?\b', question)
        if match:
...
SYSTEM_PROMPT = """You are a security operations assistant.
...
CONTEXT (from SOC knowledge base):
{context_text}
