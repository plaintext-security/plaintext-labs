...
This is the same teaching harness as the defensive/08-detection-as-code module, adapted
for ZT access log structure (Pomerium/Cloudflare Access-shaped JSONL). It supports:
...
In production you would `sigma convert` the same rule to your SIEM query language
(Splunk, Elastic, Chronicle, etc.) — the rule is identical either way.

Key Zero Trust note: because EVERY request is logged (not just anomalies), the
signal-to-noise ratio for behavioural rules is much higher than in traditional
perimeter environments. The auth flood and unexpected-country rules in this lab
produce clean signals precisely because baseline is dense and structured.
"""
import json
import sys
    ...
    with open(sys.argv[2]) as fh:
        for lineno, line in enumerate(fh, 1):
            ...
            if evaluate(event, detection):
                ...
                print(
                    f"  [HIT] line {lineno}: {ts}  user={user}  "
                    f"country={country}  service={service}  type={event_type}"
                )
