#!/usr/bin/env bash
# Seed the target account configuration (IAM + S3) against the selected AWS endpoint.
# `aws` honors AWS_ENDPOINT_URL, so this runs against floci (default) or real AWS unchanged.
# Called automatically by `make up` via docker-compose.
set -euo pipefail

echo "==> Seeding the target account IAM configuration..."

# ----- Users -----
aws iam create-user --user-name admin-svc 2>/dev/null || true
