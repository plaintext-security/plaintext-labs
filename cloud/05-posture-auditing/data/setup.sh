...
$ALIAS s3api create-bucket --bucket inherited-public-data \
...
$ALIAS iam create-user --user-name svc-legacy 2>/dev/null || true
$ALIAS iam create-access-key --user-name svc-legacy \
  --output json > /tmp/legacy-key.json 2>/dev/null || true

echo "   -> IAM user svc-legacy has an access key (never rotated)"

# ----- CloudTrail: trail exists but logging is disabled (CIS 3.1) ------------
echo "[4/5] Creating CloudTrail trail with logging off..."
$ALIAS s3api create-bucket --bucket cloudtrail-logs \
  --region $REGION 2>/dev/null || true
...
echo "   S3:           inherited-public-data  (public ACL)"
echo "   Security Group: $SG_ID  (0.0.0.0/0 on 22, 3389)"
echo "   IAM User:     svc-legacy  (stale access key)"
echo "   CloudTrail:   trail  (logging disabled)"
