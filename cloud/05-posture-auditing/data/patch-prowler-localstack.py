#!/usr/bin/env python3
"""Make Prowler 4.x usable against LocalStack.
...
STS validation (and therefore the whole scan) is routed at LocalStack. It is a
...
replacement = (
    ...
    '        # LocalStack instead of the hardcoded public AWS STS endpoint.\n'
    '        import os as _os\n'
...
if replacement.splitlines()[3] in src:
    print("patch-prowler-localstack: already applied")
    sys.exit(0)

if anchor not in src:
    sys.exit(
        "patch-prowler-localstack: STS anchor not found — prowler version drift; "
        "review providers/aws/aws_provider.py create_sts_session()"
    )

open(target, "w", encoding="utf-8").write(src.replace(anchor, replacement))
print(f"patch-prowler-localstack: patched {target}")
