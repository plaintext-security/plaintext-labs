...
- `data/incident-findings.md` — the raw forensic findings from the investigation
...
The incident is closed. You are writing the final forensic incident report,
...
1. [ ] **Read `data/incident-findings.md` in full.** Identify which findings are:
...
- [ ] IOC table is populated with at least five IOCs from the investigation.
- [ ] `lint_report.py` reports zero structural issues.

## Deliverables

Commit to your portfolio repo:
- `incident-report.md` — the completed report using the template.
...
- Add a "Detection Coverage Analysis" appendix: for each ATT&CK technique identified in the
  report, state whether the organization had a detection in place at the time, whether it fired, and
