...
incident: "Meridian Financial — workstation compromise & cloud pivot"
...
timeline:
  - time: "2024-03-14T22:14:07Z"
    event: "Attacker authenticates as jchen over VPN from 203.0.113.66"   # SUPPORTED (auth.log)
  - time: "2024-03-14T22:18:33Z"
    event: "Dropper svchost32.exe executed on WS-FIN-07"                   # SUPPORTED (4688)
  - time: "2024-03-14T23:55:00Z"
    event: "12 GB exfiltrated from S3 bucket meridian-fin-reports"          # UNSUPPORTED (no such artifact)
  - time: "2024-03-14T22:47:09Z"
    event: "Attacker creates IAM access key for persistence"               # SUPPORTED (cloudtrail)
root_cause: "Brute-force attack against the VPN gateway cracked jchen's weak password, giving initial access."
---

# Incident Summary — Meridian Financial (DRAFT, AI-generated)

## Overview

On 14 March 2024, an external actor gained access to Meridian's environment and
deployed a dropper (`svchost32.exe`) on workstation WS-FIN-07, then pivoted into
the AWS environment using jchen's session.

## Initial Access

The actor exploited **CVE-2024-3094** in the VPN gateway to obtain initial access,
following a **brute-force attack (T1110)** that cracked jchen's weak password. Once
...
The actor assumed the FinAnalyst role and, at 23:55 UTC, **exfiltrated approximately
12 GB from the S3 bucket `meridian-fin-reports`**, then created an IAM access key at
22:47 UTC for durable access.

## Root Cause

The root cause was a **brute-force attack against the VPN gateway** that cracked
jchen's weak password. Recommend enforcing MFA and account lockout.
